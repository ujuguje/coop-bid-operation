...
IDENTICAL to algorithms/coop_sac.py EXCEPT the QMIX mixer uses FixedHypernet,
which applies .abs() to BOTH mixing-weight layers (w1 AND w_final) to enforce
QMIX monotonicity  d Q_tot / d Q_local >= 0.  The shipped Hypernet (networks.py)
applies .abs() only to w1 (line 115 missing it), breaking monotonicity.

This is the ONLY change for coop_sac:
  #1 Cap_Pcs_Ope=tol+2  -> already in run_training.py / set by the runner below
  #2 no_grad removed    -> N/A (coop_sac actor update has no no_grad bug)
  #3 lmbda detach       -> N/A (SAC has no TD3+BC lmbda)
  #4 w_final.abs()      -> THIS FILE (FixedHypernet)
Training entry point: code/run_coop_dasac.py (checkpoints -> results/coop_dasac/).
"""
import os
import time
import numpy as np
...
from models.networks import SquashedGaussianMLPActor, MLPQFunction   # <-- Hypernet NOT imported
...
def train(env_: DualAgentEnv,
    ...
    mixer1  = FixedHypernet(global_dim, n_agents).to(device)   # <-- FixedHypernet
    mixer2  = FixedHypernet(global_dim, n_agents).to(device)   # <-- FixedHypernet
