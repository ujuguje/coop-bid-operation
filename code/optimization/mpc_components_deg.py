...
be shown with a bid/ope/deg split (like OJPD / Coop-DABC) in the deg sensitivity.
Saves results/sensitivity/mpc_components_deg.json. Runs ~30-40 min per deg.
deg=5.0 is computed first as a self-check against the known tol5 value (4,456)."""
...
TOL = 5.0
OUTP = os.path.join(SENS, "mpc_components_deg.json")
out = {}
# deg=5.0 first as a self-check against known tol5 value (Total ~4456)
for deg in [5.0, 2.5, 7.5, 10.0]:
    degc = deg*0.25   # per-slot (15-min) degradation rate, matches env
    t0=time.time(); daily=run_mpc(TOL, degc); a=np.array(daily)
    rec = dict(deg=deg, bid=float(a[:,0].mean()), ope=float(a[:,1].mean()),
               deg_cost=float(a[:,2].mean()), total=float(a[:,3].mean()))
    out[str(deg)] = rec
    print(f"deg={deg:>5}  Bid={rec['bid']:,.0f}  Ope={rec['ope']:,.0f}  Deg={rec['deg_cost']:,.0f}  Total={rec['total']:,.0f}  ({time.time()-t0:.0f}s)", flush=True)
    json.dump(out, open(OUTP,"w"), indent=2)  # save incrementally
print("MPC deg components done ->", OUTP, flush=True)
